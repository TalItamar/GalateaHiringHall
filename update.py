...
if st.session_state.get("authentication_status"):
    ...
    if contract:
        ...
        **Command Rights:** {contract[0][10]}
        ---""")
        # Check if user already has an active contract for this contract name
        user_contracts = get_company_contracts(username)
        already_negotiated = any(c[2] == contract[0][2] and c[1] == username and c[11] == 'active' for c in user_contracts)
        if already_negotiated:
            st.info("You have already negotiated this contract. You cannot negotiate it again.")
            st.button("Negotiate Contracts", disabled=True)
        else:
                ...
                def negotiation():
                    ...
                    if st.button("Submit Negotiation", disabled=not can_submit, key="submit_negotiation"):
                        ...
                        add_contract(
                            ...
                            contract[0][3],  # length
                            contract[0][4],  # tracks
                            contract[0][5],  # track_types
                            int(step_to_value("Base Pay", new_steps["Base Pay"])),
